      ...
      <div className="hidden md:flex w-1/2 bg-rosewood-700 flex-col items-center justify-center p-xl relative overflow-hidden">
        ...
        >
          ...
          <p className="font-headline-md text-on-primary/80 mt-md max-w-xs text-center">
            Discover recipes from the ingredients you love
          </p>
        </motion.div>
      </div>

      {/* Right panel — form */}
      <div className="w-full md:w-1/2 flex items-center justify-center p-margin-mobile md:p-xl bg-background">
        <motion.div
          initial={{ opacity: 0, y: 24, scale: 0.97 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          transition={{ type: 'spring', stiffness: 260, damping: 26 }}
          className="w-full max-w-md bg-surface rounded-2xl border border-blush shadow-[0_8px_30px_rgb(92,61,46,0.08)] p-lg flex flex-col gap-md"
        >
          {/* Mobile logo */}
          ...
          <h2 className="font-headline-lg text-deep">Welcome Back</h2>

          <LoginForm />

          <p className="text-center font-label-lg text-deep-muted">
            Don't have an account?{' '}
            <Link to="/signup" className="text-terra-dark hover:underline font-label-lg">
              Sign up
            </Link>
          </p>
        </motion.div>
